...
void help_message()
{
  cerr<< "Argumets: ./leadCut /path/to/input/skim/file /path/to/output/root/file [nucleon of interest]\n\n"
      ...
      <<"-x: Set minimum xB (default = 1.2)\n"
      <<"-p: Set minimum pMiss [GeV] (default = 0.3)\n\n";
}
...
int main(int argc, char ** argv){
  ...
  if(parOfInt == 1){
    ...
    cerr<<"You chose an incorrect nucelon of interest.\n Aborting...\n\n";
    exit(-2);
  }
  bool verbose = false;
  double minPMiss = 0.3;
 ...
 int numDouble = 0;
  ...
  for(int i = 0; i < fin; i++){
	...
	vtxZCorr[l] = myInfo.getVTX(l);
      ...
      else if(myInfo.isNeutron(1)){numLeadN++;}
      outTree->Fill();
      nEvents++;
    }
    else if(leadIndex == -2){
