...
// The reaosn for not weighting the whole set is that some data has poor events in certain regions (bias)
// We are also plotting the P_Miss
...
// Make Histograms of Missing Momentum to Compare
  const int total_bins = 20.; 
  const int lower_bound = 50.;
  const int upper_bound = 650.;
  const int scale_begin = 450.;
  const int scale_end = upper_bound;
...
// Create input and output files for MF and SRC
...
// Create output Trees for new weights
  TTree * outtree_weights = new TTree("updated_weights", "Weights");  // Will add MF and SRC trees as "friends"
  TTree * MF_Tree_Weights = new TTree("updated_MF_weights", "MF Weights");
  TTree * SRC_Tree_Weights = new TTree("updated_SRC_weights", "SRC Weights");
...
// Define Variables for Loop
  ...
  double_t weight_sum[2], new_weight, scale;;
...
// Scale should be in a well known region as it will be extended to whole data set
  std:: cout << "Looking at Small Momentum Segment Between: [" << scale_begin << " , " << scale_end << "] MeV\n";
...
// Making histograms of missing momentum to compare to SRC/MF data
  ...
  for(int i = 0; i < Real_Tree->GetEntries(); i++){
    ...
    if (Xb > 1.6 or Xb < 1.) continue;
    if (Q2 > 4. or Q2 < 0.5) continue;
    ...
    if (P_miss_real.Mag()*1000 < scale_begin or P_miss_real.Mag()*1000 > scale_end) continue;
    Real_DATA_Entries +=1.;}
...
// Loop over all entries to find the total sum of the weight. Not saving the values yet
  ...
  for (int i = 0; i < total_entries; i++){
    ...
    // Get missing momentum (MF part is arbitrary)
    if (applied_simulator){
    ...
    if (P_miss_MF.Mag()*1000 < scale_begin or P_miss_MF.Mag()*1000 > scale_end) continue;
