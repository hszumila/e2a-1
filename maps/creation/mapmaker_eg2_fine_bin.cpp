...
int main(int argc, char ** argv)
{
  ...
  generated = new TH3D("solid_e_gen","solid_e_gen",pbins,0,5,costbins,5,25,phibins,-30,330);
  accepted = new TH3D("solid_e_acc","solid_e_acc",pbins,0,5,costbins,5,25,phibins,-30,330);
  ...
  int particle_g[maxPart], particle[maxPart], num_g;
  ...
  for (int file = 0; file < numfiles; file++)
    {
      ...
      for(int event = 0; event<intree->GetEntries();event++)
	    ...
	    continue;
          if (particle_oi == "e")
            {
              generated->Fill(mom_g[0],theta_g[0],phi_g[0]);
		...
		continue;
              ...
              accepted->Fill(mom_g[0],theta_g[0],phi_g[0]);
            }
