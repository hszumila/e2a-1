...
const double corr_pp[3] = {0.6621,0.7795,0.2484};//bg subtraction for e,e'pp
...
int main(){
  ...
  TString outputpdf = Form("out_neff/output_%.1f_corr",Ebeam);
  // --------------------------------------------------------------------------
  ...
  TF1 *fitFcn_ppin1 = new TF1("fitFcn_ppin1",fitSB,0.7,1.3,7);
  fitFcn_ppin1->SetParameters(100,-300,300,-300,150,0.9,0.05);
  fitFcn_ppin1->SetParLimits(5,0.85,0.95);
  h1_Mmiss_eppin_inc1->Fit("fitFcn_ppin1","R");
  TF1 *fSignal1pi = new TF1("fSignal1pi",gaussianPeak,0.7,1.3,3);
  fSignal1pi->SetLineColor(kBlue);
  TF1 *fBackground1pi = new TF1("fBackground1pi",background,0.7,1.3,4);
  Double_t param1pi[7];
