...
int main(){
  ...
  TFile *f = new TFile("../../../data/nina/skim_C12_p.root");
  ...
  //for (int event =0 ; event < 100000 ; event++)
	...
	{   
	  ...
	  double theta_curr = this_mom.Angle(q)*180.0/M_PI;
	  //if (theta_curr>25.0){continue;}

	  //first proton of interest in event
	  if (leadingID==-1){p_over_q_prev = p_over_q; theta_prev = theta_curr; leadingID=part;}

	  //////////////////////////////////////////////////////////////////////////////////
	  //Here I meticulously selected the proton most likely to pass lead if more than 1:
	  //current p/q is larger than previous
	  if (p_over_q>p_over_q_prev){
	    
	    //current angle also the smallest
	    if (theta_curr<theta_prev){leadingID=part;}

	    //current angle is larger than previous:
	    else {	      
	      //cout<<"     p/q current: "<<p_over_q<<" theta_pq: "<<theta_curr<<endl;
	      //cout<<"     p/q previous: "<<p_over_q_prev<<" theta_pq: "<<theta_prev<<endl;
	      
	      //both pass p/q, one passes theta
	      if (p_over_q>0.62 && p_over_q_prev>0.62 && (theta_curr>25.0 || theta_prev>25.0)){
		if (theta_curr<theta_prev){leadingID=part;}
	      }	      
	      //both pass theta, one passes p/q
	      else if ((p_over_q<0.62 || p_over_q_prev<0.62) && theta_curr<25.0 && theta_prev<25.0){
		if(p_over_q>p_over_q_prev){leadingID=part;}
	      }
	      //one or both passes theta or p/q
	      else if ((p_over_q>0.62||theta_curr<25.0) || (p_over_q_prev>0.62||theta_prev<25.0)){
		if ((p_over_q>0.62||theta_curr<25.0) && (p_over_q_prev<0.62&&theta_prev>25.0)){leadingID=part;}
		else if ((p_over_q>0.62||theta_curr<25.0) && (p_over_q_prev>0.62||theta_prev<25.0)){
		  if(abs(p_over_q-0.62)/0.62*abs(theta_curr-25)/25.0 < abs(p_over_q_prev-0.62)/0.62*abs(theta_prev-25)/25.0){leadingID=part;}
		} 
	      }
	      //neither passes both theta nor p/q
	      else if (abs(p_over_q-0.62)/0.62*abs(theta_curr-25)/25.0 < abs(p_over_q_prev-0.62)/0.62*abs(theta_prev-25)/25.0){leadingID=part;} 
	    }
	  }//end lead proton selection
	  /////////////////////////////
	}
      ...
      if (leadingID > 0)
	{
	  ...
	  //vertex cut-carbon, reminder there's no actual lead proton cuts here
	  if (vtxZ[leadingID]>4 && vtxZ[leadingID]<7){
	    //passing lead proton cuts
	     ...
	     if (p_over_q>0.62&&p_over_q<1.1&&theta_curr<25.0&&mom_miss.Mag()<1.0&&mmiss<1.1){
	      h_xb->Fill(xB);
	      h_nu->Fill(Nu);
  ...
  for (int ii=0;ii<7; ii++){
	...
	if (pmissSmear[kk]>pmiss_cut[jj] && pmissSmear[kk]<1.0 && mmissSmear[kk]<mmiss_val[ii] && p_over_q_sm[kk] >= 0.62 && p_over_q_sm[kk] <= 1.1 && thetaPQsm[kk]<25.0*M_PI*180.0){
	  den++;
	  //false positive: non-smeared fails, smeared passes
	  if(pmissO[kk]<0.3 || pmissO[kk]>1.0 || mmissO[kk]>1.1 || p_over_q_O[kk] < 0.62|| p_over_q_O[kk] > 1.1 || thetaPQ[kk]>25.0*M_PI*180.0){
	    fpos++;
	  }
	...
	if((pmissO[kk]>0.3 && pmissO[kk]<1.0 && mmissO[kk]<1.1 && p_over_q_O[kk] >= 0.62 && p_over_q_O[kk] <= 1.1 && thetaPQ[kk]<25.0*M_PI*180.0)
	   &&  (pmissSmear[kk]<pmiss_cut[jj] || mmissSmear[kk]>mmiss_val[ii]|| pmissSmear[kk]>1.0 || p_over_q_sm[kk] < 0.62 || p_over_q_sm[kk] > 1.1 || thetaPQsm[kk]>25.0*M_PI*180.0)){
	  fneg++;
	}
	...
	//Offset Calculation///////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////
	if (pmissO[kk]>pmiss_cut[jj] && pmissO[kk]<1.0 && mmissO[kk]<mmiss_val[ii] && p_over_q_O[kk] >= 0.62 && p_over_q_O[kk] <= 1.1 && thetaPQ[kk]<25.0*M_PI*180.0){
	  denSub++;
	  //false positive: non-smeared fails, smeared passes
	  if(pmissO[kk]<0.3 || pmissO[kk]>1.0 || mmissO[kk]>1.1 || p_over_q_O[kk] < 0.62|| p_over_q_O[kk] > 1.1 || thetaPQ[kk]>25.0*M_PI*180.0){
	    fposSub++;
	...
	//false negative: smeared fails, non-smeared passes
	if((pmissO[kk]>0.3 && pmissO[kk]<1.0 && mmissO[kk]<1.1 && p_over_q_O[kk] >= 0.62 && p_over_q_O[kk] <= 1.1 && thetaPQ[kk]<25.0*M_PI*180.0)
	   &&  (pmissO[kk]<pmiss_cut[jj] || mmissO[kk]>mmiss_val[ii]|| pmissO[kk]>1.0 || p_over_q_O[kk] < 0.62 || p_over_q_O[kk] > 1.1 || thetaPQ[kk]>25.0*M_PI*180.0)){
	  fnegSub++;
	}
      ...
      falsePosSub[ii][jj] = fpos*100.0/den - fposSub*100.0/denSub;
      falsePosOff[ii][jj] = fposSub*100.0/denSub;
      //false -ive: events that should pass do not pass
      falseNeg[ii][jj] = fneg*100.0/den;
      falseNegSub[ii][jj] = fneg*100.0/den - fnegSub*100.0/denSub;
      falseNegOff[ii][jj] = fnegSub*100.0/denSub;
